...
def generate_image():
    ...
    try:
        ...
        response = openai.Image.create(
            prompt=enhanced_prompt, n=1, size=size, response_format="b64_json"
        )
...
def generate_video():
    ...
    try:
        ...
        payload = {
            ...
            "input": {"prompt": prompt, "duration": duration, "seed": -1},
        }

        response = requests.post(
            "https://api.replicate.com/v1/predictions", headers=headers, json=payload
        )
    ...
    try:
        ...
        if status == "succeeded" and prediction.get("output"):
            ...
            if video_response.status_code == 200:
                ...
                # Generate HTML snippet
                metadata = {
                    "prompt": prediction.get("input", {}).get("prompt", "No prompt")
                }
...
def ai_chat():
    ...
    try:
        ...
        current_app.chat_sessions[session_id].append(
            {"role": "user", "content": message}
        )

        # Keep only last 10 messages for context
        if len(current_app.chat_sessions[session_id]) > 10:
            current_app.chat_sessions[session_id] = current_app.chat_sessions[
                session_id
            ][-10:]
        ...
        return jsonify(
            {"success": True, "response": ai_response, "session_id": session_id}
        )
...
def call_twiml():
    ...
    try:
        ...
        if call_id and hasattr(current_app, "call_messages"):
            message = current_app.call_messages.get(
                call_id, "Hello from GEM Enterprise."
            )
        else:
