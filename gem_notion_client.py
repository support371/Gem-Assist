...
class NotionLeadershipClient:
    def __init__(self):
        ...
        self.client: Client = Client(auth=self.notion_token)
        logger.info(
            f"Notion client initialized with database ID: {self.database_id[:8]}..."
        )
    ...
    def get_leadership_data(self):
        ...
        try:
            ...
            logger.info(
                f"Successfully fetched {len(leadership_data)} leadership entries"
            )
    ...
    def _extract_leader_info(self, page):
        ...
        try:
            ...
            leader = {
                "id": page["id"],
                "name": self._get_title_property(
                    properties, ["Name", "Full Name", "Title"]
                ),
                "position": self._get_text_property(
                    properties, ["Position", "Title", "Role"]
                ),
                "bio": self._get_text_property(
                    properties, ["Bio", "Biography", "Description"]
                ),
                "email": self._get_email_property(properties, ["Email", "Contact"]),
                "linkedin": self._get_url_property(
                    properties, ["LinkedIn", "LinkedIn URL"]
                ),
                "experience": self._get_text_property(
                    properties, ["Experience", "Years Experience"]
                ),
                "expertise": self._get_multi_select_property(
                    properties, ["Expertise", "Specialties", "Skills"]
                ),
                "order": self._get_number_property(properties, ["Order", "Sort Order"]),
                "photo_url": self._get_file_property(
                    properties, ["Photo", "Image", "Picture"]
                ),
                "featured": self._get_checkbox_property(
                    properties, ["Featured", "Is Featured"]
                ),
            ...
            if name in properties:
                ...
                if prop["type"] == "files" and prop["files"]:
                    ...
                    return (
                        ...
                        else prop["files"][0]["external"]["url"]
                    )
        return ""
