  ...
  try {
    ...
    if (process.env.REPLICATE_API_TOKEN) {
      const replicateResponse = await axios.post(
        ...
        {
          version:
            "009101d209a72e8e7e6bb1cd28e8be0ebd53d7c7f3968cb670b3e9ac8e7acc5b", // Stable Video Diffusion
          input: {
            cond_aug: 0.02,
            decoding_t: 7,
      ...
      do {
        ...
        (prediction.status === "processing" && attempts < maxAttempts)
      );
      ...
      if (prediction.status === "succeeded" && prediction.output) {
        ...
        throw new Error(
          `Video generation failed with status: ${prediction.status}`,
        );
      }
    } else {
...
app.get("/api/media/twiml/:filename", async (req, res) => {
  try {
    ...
    log("error", "TwiML serving failed", {
      error: error.message,
...
app.use(
  "/generated-assets",
  express.static(path.join(__dirname, "generated-assets")),
);
