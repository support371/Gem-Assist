...
class ModernEnterpriseFeatures {
  ...
  animateProgressCircle() {
    ...
    if (circle) {
      const percent =
        document.querySelector(".progress-circle").dataset.percent || 98.7;
  ...
  simulateLiveData() {
    ...
    const threatCounter = document.querySelector(
      '.threat-stats strong[data-count="1247"]',
    );
    if (threatCounter) {
      setInterval(() => {
        const currentValue = parseInt(
          threatCounter.textContent.replace(",", ""),
        );
  ...
  setupScrollAnimations() {
    const sections = document.querySelectorAll(
      ".modern-services-section, .dashboard-container",
    );
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            const children = entry.target.querySelectorAll(
              ".service-card-modern, .dashboard-card",
            );
...
class MultimediaGalleryManager {
    ...
    mediaTabs.forEach((tab) => {
      tab.addEventListener("click", (e) => {
        ...
        if (parentGallery) {
          ...
          e.target.classList.add("active");
          const targetContent = parentGallery.querySelector(
  ...
  setupVideoPlayers() {
    const videoPlaceholders = document.querySelectorAll(
      ".video-placeholder, .preview-btn",
    );

    videoPlaceholders.forEach((placeholder) => {
      placeholder.addEventListener("click", (e) => {
        const videoId =
          e.target.closest("[data-video]")?.dataset.video ||
  ...
  openVideoModal(videoId) {
    ...
    requestAnimationFrame(() => {
      ...
      modal.querySelector(".modal-content-multimedia").style.transform =
        "scale(1)";
    });
  }
  ...
  setupMediaActions() {
    const mediaActionBtns = document.querySelectorAll(
      ".media-action-btn, .preview-btn",
    );
  ...
  openImageGallery() {
    ...
    requestAnimationFrame(() => {
      modal.style.opacity = "1";
      modal.querySelector(".gallery-content-multimedia").style.transform =
        "scale(1)";
    });
  }
  ...
  openCaseStudy() {
    ...
    requestAnimationFrame(() => {
      modal.style.opacity = "1";
      modal.querySelector(".case-study-content-multimedia").style.transform =
        "scale(1)";
    });
  }
